...
class Crea_Perso:
    ...
    def classe(self):
        ...
        if self.race=='Elfe':
            ...
            while self.classe not in lclasse:
                ...
                self.classe=str(input())
        if self.race=='Humain':
            lclasse=['Pyroguerrier','Coeurdeglace','Assassin','Paladin','Necromancien']
            print("Choisissez votre classe:")
            print(lclasse)
            self.classe=str(input())
            while self.classe not in lclasse:
    ...
    def PV(self):
        self.pv=random.randrange(100,200,10)
    ...
    def Force(self):
        self.F=random.randrange(10,100,10)
    ...
    def arme_force(self):
        if self.arme=='Epee batarde' or self.arme=='Hache de guerre' or self.arme=='Hallbarde':
            self.arme_force=random.randrange(10,40,10)
        elif self.arme=='Sceptre':
            self.arme_force=0
        else:
            self.arme_force=random.randrange(10,60,10)
    ...
    def arme_mat_dmg(self):
        if self.arme_mat=='Diamant' or 'Acier':
            self.arme_mat_dmg=random.randrange(10,200,10)
        elif self.arme_mat=='Fer' or 'Onyx':
            self.arme_mat_dmg=random.randrange(10,120,10)
        else:
            self.arme_mat_dmg=random.randrange(10,100,10)
    ...
    def arme_totale(self):
        ...
        if arme_t[0]<arme_t[1]:
            self.arme_totale=arme_t[1]-arme_t[0]
        else:
            self.arme_totale=arme_t[0]-arme_t[1]

        print (f'Votre {self.arme} de {self.arme_mat} à une puissance de {self.arme_totale} de Force' )
    ...
    def bonus(self):
        ...
        for i in range(3):
            ...
            self.bonus=all_liste.liste_bonus()[f'{r}']
            print (f'-{self.bonus}')
        return self.bonus
    ...
    def malus(self):
        ...
        for i in range(3):
            r=random.randint(1,15)
            self.malus=all_liste.liste_malus()[f'{r}']
            print (f'-{self.malus}')
        return self.malus

    def liste_bonus_guerrier(self):
        all_liste.liste_bonus_guerrier()
        self.liste_bonus_guerrier=all_liste.liste_bonus_guerrier()[self.classe]
        print(f'Votre compétence unique de {self.classe} est {self.liste_bonus_guerrier}')
    ...
    def competence(self):
        ...
        comp_r_f=random.randint(0,7)
        print('Voici votre 4 compétences:')

        if comp_r_f==3:
            print(f'-{all_liste.liste_competence_faible()[comp_r_f]} {all_liste.liste_complement(self)[0]} ')
        else:
            print(f'-{all_liste.liste_competence_faible()[comp_r_f]} {all_liste.liste_element()[self.classe]}')

        comp_r_m=random.randint(0,16)

        if comp_r_m==3:
            print(f'-{all_liste.liste_complement(self)[1]}{all_liste.liste_competence_moyen()[comp_r_m]} ')
        else:
            print(f'-{all_liste.liste_competence_moyen()[comp_r_m]} {all_liste.liste_element()[self.classe]}')
        ...
        if comp_r_m==3:
            ...
            print(f'-{all_liste.liste_competence_moyen()[comp_r_m]} {all_liste.liste_complement(self)[2]} ')

        else:
            print(f'-{all_liste.liste_competence_moyen()[comp_r_m]} {all_liste.liste_element()[self.classe]}')

        comp_r_p=random.randint(0,9)
        if comp_r_p==1 or comp_r_p==5:
            print(f'-{all_liste.liste_competence_puissant()[comp_r_p]} {all_liste.liste_complement(self)[2]} ')
        else:
            print(f'-{all_liste.liste_competence_puissant()[comp_r_p]} {all_liste.liste_element()[self.classe]}')
    ...
    def affichage(self):
        ...
        pygame.display.set_caption(f"Fiche de {self.nom}")
        screen=pygame.display.set_mode((560,790))

        running=True

        self.affichage=pygame.image.load('assets/BG_/Vincent Valentine -Wallpaper- - DarkNika-Création.png')

        while running:

            screen.blit(self.affichage,(0,0))
